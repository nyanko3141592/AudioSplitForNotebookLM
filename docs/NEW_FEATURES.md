...
### 📋 機能概要
背景情報欄に「知識プリセット」ウィジェットを追加し、会議種別ごとの定型文をドラッグ不要で挿入できるようにしました。商談・社内定例・サポート振り返りなどの既定テンプレートに加え、ユーザー独自のテンプレも無制限に登録できます。

### 🧩 UIの特徴
- **コンパクトリスト**: デフォルトはタイトルのみ表示。`>` ボタンで詳細を縦展開し、概要＋本文を確認。
- **複数選択＆一括挿入**: チェックボックスで複数プリセットを選び、「選択内容を背景情報に追加」でまとめて挿入。重複行は自動的に除外されます。
- **共有選択状態**: 文字起こしステップで選んだプリセットは議事録ステップでも同じ状態で表示され、追加・解除が簡単。
- **操作系**: 「クリア」で選択のみ解除、背景テキストは維持。処理中は自動的に入力がロックされます。

### 🛠️ プリセット管理フロー
1. **作成**: 「プリセットを作成」→ 名前／概要（任意）／本文を入力して保存。
2. **編集**: ✏️ アイコンから内容を更新。標準プリセットも編集するとローカル上書きとして保存されます。
3. **削除**: 🗑️ でプリセットをローカルから削除。標準プリセットでも削除可能です。
4. **永続化**: 作成・上書き・削除状態は LocalStorage に保存され、ブラウザを閉じても維持されます。

### 🔁 ワークフロー連携
- `TranscriptionStep` と `SummaryStep` が `selectedKnowledgePresetIds` を共有することで、文字起こしで選んだプリセットが議事録作成時にも引き継がれます。
- 背景情報に挿入されたテキストは通常の入力欄で編集でき、不要な場合はチェックを外す＋テキストを削除するだけ。

### ✅ 活用シナリオ
- **営業商談**: 案件概要、意思決定者、交渉ポイントをテンプレ化してワンクリック挿入。
- **社内定例**: 参加部署やKPIリスト、今週のトピックをストックして毎回の抜け漏れを防止。
- **サポート振り返り**: 観測期間・主要指標・エスカレーション情報を定型化し、要約精度を高める。
